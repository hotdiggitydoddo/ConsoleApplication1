...
public	enum Tile{None, Floor, RoomFloor, Wall, Door, OpenDoor, ClosedDoor, DoorNorth, DoorEast, DoorSouth, DoorWest };
...
public class DungeonGenerator {
	...
	*/
	...
	public void setTile(IntVector2 pos, Tile tile)	{	_dungeon[pos.x,pos.y] = tile;			}	//OCCASIONAL ERROR
	public Tile getTile(IntVector2 pos)				{	return _dungeon[pos.x,pos.y];			}
	...
	public class RoomTemplate/*, System.IComparable<RoomTemplate>*/{
		...
		public RoomTemplate(Tile[,] tiles, int x, int y, List<Door> potentialDoors){			
			this.tiles	= tiles;
			this.x		= x;			
			this.y		= y;
	...
	//			
				...
				if(!collision){	//create actual room
					...
					_dungeon[placedRoom.x+jackDoor.x, placedRoom.y+jackDoor.y] = Tile.Door;	//retroactively carve door into dungeon, placedRoom has been carved before
										
					//2x2 Door
					if(jackDoor.doorLength > 1){
						newRoom.tiles		[plugDoor.x	+(verticalDoor?1:0), plugDoor.y	+(verticalDoor?0:1)] = Tile.Door;
						placedRoom.tiles[jackDoor.x	+(verticalDoor?1:0), jackDoor.y	+(verticalDoor?0:1)] = Tile.Door;
						_dungeon[placedRoom.x+jackDoor.x+(verticalDoor?1:0), placedRoom.y+jackDoor.y+(verticalDoor?0:1)] = Tile.Door;												
					}
			...
			if(!collision){
				...
				for(int x = 0; x < xSizeRoom; x++){
					for(int y = 0; y < ySizeRoom; y++){						
						if(newRoom.tiles[x,y] != Tile.None){	//copy everything except Tile.None into dungeon							
							_dungeon[x +offX,y +offY] = newRoom.tiles[x,y];							
							if(newRoom.tiles[x,y] == Tile.Floor){	_regions[x +offX,y +offY] = currentRoom;	}	//set region too					
						}
					}
				}
				currentRoom++;
	...
	private	Room CreateRoomFromTemplate(RoomTemplate roomTemplate){
		...
		room = new Room(roomTemplateTilesCopy, roomTemplate.x, roomTemplate.y, new List<Door>());		
	...
	private	RoomTemplate GenerateRandomRoomTemplate(){
		...
		//create new room
		roomTemplate.tiles = CreateUnborderedRectangleRoom(xSize, ySize);
		...
		for(int i = 0; i< overlayCount; i++){
			if(Random.Next(0,101)<overlayChance){	roomTemplate.tiles = OverlayNewRoom( roomTemplate.tiles );	}
		}
	...
	private	Tile[,] CreateBorderedRectangleRoom(int xSize, int ySize){
	...
	private	Tile[,] CreateUnborderedRectangleRoom(int xSize, int ySize){		
		Tile[,]	tiles = new Tile[xSize,ySize];						
		//insides
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){			
	...
	private	void IngrowRoomBorderTiles( Tile[,] roomTiles ){	//D array is passed by ref
	...
	private	Tile[,] OverlayNewRoom(Tile[,] oldTiles){
		...
		//ovelay room tiles
		Tile[,] overlay = CreateUnborderedRectangleRoom(xSizeOverlay, ySizeOverlay);
			...
			for(int y = 0; y < newYSize; y++){
				...
				if(x >= offX && y >= offY){									// if on or past the origin of overlay
					if(x < offX +xSizeOverlay && y < offY +ySizeOverlay){	// if room is completely contained we have to check if the overlay array isnt out of bounds
						...
						if			( oldTile == Tile.None ){				//if new is overlaying None: replace
							newTiles[x,y] = overTile;
						}else if	( oldTile == Tile.Floor || overTile == Tile.Floor){	//floor always wins, overrwriting old walls too if walls already exist on starting room
	...
	private	int adjacentWalls( Tile[,] tiles, int x, int y){
		int adj = 0;
		if( tiles[x-1,y+0] == Tile.Wall ){adj++;}
		if( tiles[x+0,y+1] == Tile.Wall ){adj++;}
		if( tiles[x+1,y+0] == Tile.Wall ){adj++;}
		if( tiles[x+0,y-1] == Tile.Wall ){adj++;}
		return adj;
	}
	...
	private	void FillRoomCavities( RoomTemplate roomTemplate ){
		...
		for(int x = 1; x < xSize-1; x++){
			for(int y = 1; y < ySize-1; y++){
				if(	roomLabels[x,y] != largestRegionLabel && roomLabels[x,y]!= 0 ){	roomTemplate.tiles[x,y]= Tile.Wall;	}
			}
		}		
	}
	...
	private int FillNeighbors(ref Tile[,] tiles, int label, int x, int y, ref int[,] roomLabels, int depth){
		...
		if(roomLabels[x+0, y+1] == 0 && tiles[x+0, y+1] == Tile.Floor){	hits += FillNeighbors(ref tiles, label, x+0, y+1, ref roomLabels, depth);	}
		if(roomLabels[x+1, y+0] == 0 && tiles[x+1, y+0] == Tile.Floor){	hits += FillNeighbors(ref tiles, label, x+1, y+0, ref roomLabels, depth);	}
		if(roomLabels[x+0, y-1] == 0 && tiles[x+0, y-1] == Tile.Floor){	hits += FillNeighbors(ref tiles, label, x+0, y-1, ref roomLabels, depth);	}
		if(roomLabels[x-1, y+0] == 0 && tiles[x-1, y+0] == Tile.Floor){	hits += FillNeighbors(ref tiles, label, x-1, y+0, ref roomLabels, depth);	}
		return hits;
	}
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				...
				if( roomTemplate.tiles[x,y] == Tile.Wall ){
					...
					obsolete	= obsolete&& ( ignoreN||				( roomTemplate.tiles[x+0,y+1] == Tile.Wall || roomTemplate.tiles[x+0,y+1] == Tile.None) );	//N
					obsolete	= obsolete&& ( ignoreN||	ignoreE||	( roomTemplate.tiles[x+1,y+1] == Tile.Wall || roomTemplate.tiles[x+1,y+1] == Tile.None) );	//NE
					obsolete	= obsolete&& ( ignoreE||				( roomTemplate.tiles[x+1,y+0] == Tile.Wall || roomTemplate.tiles[x+1,y+0] == Tile.None) );	//E
					obsolete	= obsolete&& ( ignoreS||	ignoreE||	( roomTemplate.tiles[x+1,y-1] == Tile.Wall || roomTemplate.tiles[x+1,y-1] == Tile.None) );	//SE
					obsolete	= obsolete&& ( ignoreS||				( roomTemplate.tiles[x+0,y-1] == Tile.Wall || roomTemplate.tiles[x+0,y-1] == Tile.None) );	//S
					obsolete	= obsolete&& ( ignoreS||	ignoreW||	( roomTemplate.tiles[x-1,y-1] == Tile.Wall || roomTemplate.tiles[x-1,y-1] == Tile.None) );	//SW
					obsolete	= obsolete&& ( ignoreW||				( roomTemplate.tiles[x-1,y+0] == Tile.Wall || roomTemplate.tiles[x-1,y+0] == Tile.None) );	//W
					obsolete	= obsolete&& ( ignoreN||	ignoreW||	( roomTemplate.tiles[x-1,y+1] == Tile.Wall || roomTemplate.tiles[x-1,y+1] == Tile.None) );	//NW
					
					if(obsolete){roomTemplate.tiles[x,y] = Tile.None;}
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				...
				if( tiles[x,y] == Tile.Floor ){
					...
					if(noneAdj && flooAdj){tiles[x,y] = Tile.Wall;}
	...
	private	void MarkPotentialDoors(RoomTemplate roomTemplate ){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if( roomTemplate.tiles[x,y] == Tile.Wall ){
					...
					if		( nTile == Tile.None  && sTile == Tile.Floor && eTile == Tile.Wall  && wTile == Tile.Wall	){	roomTemplateTilesCopy[x,y] = Tile.DoorNorth;	}	//door North
					else if	( nTile == Tile.Floor && sTile == Tile.None  && eTile == Tile.Wall  && wTile == Tile.Wall	){	roomTemplateTilesCopy[x,y] = Tile.DoorSouth;	}	//door South
					else if	( nTile == Tile.Wall  && sTile == Tile.Wall  && eTile == Tile.None  && wTile == Tile.Floor	){	roomTemplateTilesCopy[x,y] = Tile.DoorEast;		}	//door East
					else if	( nTile == Tile.Wall  && sTile == Tile.Wall  && eTile == Tile.Floor && wTile == Tile.None	){	roomTemplateTilesCopy[x,y] = Tile.DoorWest;		}	//door West
				}
			}
		}
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				...
				if(roomTemplateTilesCopy[x,y] != Tile.None && roomTemplateTilesCopy[x,y] != Tile.Wall){	//is door?
					...
					if			( roomTemplateTilesCopy[x,y] == Tile.DoorNorth	|| roomTemplateTilesCopy[x,y] == Tile.DoorSouth) {	//door North
						
						DoorDirection dir = roomTemplateTilesCopy[x,y] == Tile.DoorNorth? DoorDirection.North: DoorDirection.South;
					...
					}else if	( roomTemplateTilesCopy[x,y] == Tile.DoorEast	|| roomTemplateTilesCopy[x,y] == Tile.DoorWest) {	//door South
													
						DoorDirection dir = roomTemplateTilesCopy[x,y] == Tile.DoorEast? DoorDirection.East: DoorDirection.West;
		...
		*/				
		...
		foreach(List<IntVector2> trace in traces){
			...
			for(int i = 0; i< trace.Count; i++){
				...
				foreach(IntVector2 dir in cardinal) {					
					if(getTile( trace[i] +dir ) == Tile.Wall){			//if we see a wall in test direction
	...
	private	int buildLineSegment(IntVector2 current, ref List<IntVector2> source, ref List<IntVector2> target, int currentDepth, bool addAtEnd){
		...
		int exits = 0;
		foreach(IntVector2 dir in cardinal) {
			if (getTile(current + dir) != Tile.Wall){	//if there is anything other than a wall we have an exit or else, doorsteps will have at least 2 non walls (door + path)
				exits++;
			}
		}
		if(exits > 2){			
